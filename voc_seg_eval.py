...
test_transform = transforms.Compose([
    RandomResize_For_Segmentation(224, 224),
    Normalize_For_Segmentation(imagenet_mean, imagenet_std),
    Top_Left_Crop_For_Segmentation(224),
    Transpose_For_Segmentation()
    ])

test_dataset = VOC_Dataset_For_Testing_CAM('/data/datasets/VOCdevkit_for_cam/VOC2012/', 'train', test_transform)
...
def eval_batch(image, labels, evaluator, index):
    ...
    # segmentation test for our method
        ...
        Res = lrp.generate_LRP(image.cuda(), start_layer=1, method="transformer_attribution").reshape(batch_size, 1, 14,
                                                                                                      14)
...
predictions, targets = [], []
for batch_idx, (image, _, labels) in enumerate(iterator):
    ...
    if args.method == "blur":
        ...
        images = image.cuda()
    labels = labels.cuda()
    unused_cls_idx = labels == 255
    labels[labels != 0] = 1
    labels[unused_cls_idx] = -1
