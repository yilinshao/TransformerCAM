...
import core.arch_vit.configs as configs
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        self.merge_logits_head = Linear(4, 1)

    def forward(self, x, merge_logits=False, labels=None):
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            if posemb.size() == posemb_new.size():
                ...
                # TODO change the 2
                zoom = (gs_new / (2*gs_old), gs_new / (2*gs_old), 1)
                posemb_grid = ndimage.zoom(posemb_grid_cp, zoom, order=1)
                posemb_grid = posemb_grid.reshape(1, int(gs_new/2) * int(gs_new/2), -1)
                posemb = np.concatenate([posemb_tok, posemb_grid], axis=1)
                self.transformer.embeddings.tiled_position_embeddings.copy_(np2th(posemb))
